<script>
	...
	let playlistToHighlight = null;

	// handles songs added to playlist
	// from sidebar
	// and highlights the playlist
	$: if ($addedToPlaylist) {
		playlistToHighlight = $playlists.items.filter((f) => f.id === $addedToPlaylist.playlist)[0];

		const idx = $playlists.items.findIndex((f) => f.id === $addedToPlaylist.playlist);
		// add to track number
		$playlists.items[idx].tracks.total = $playlists.items[idx].tracks.total + 1;
		$playlists = $playlists;
		// remove the highlight class
		setTimeout(() => {
			$addedToPlaylist = null;
			playlistToHighlight = null;
		}, 800);
	}
		...
		{#each $playlists?.items || [] as playlist, i}
			{#if playlist}
				<div
					...
					class="playlist-item {playlistToHighlight === playlist
						? 'highlighted'
						: ''} transition-all p-4 hover:bg-teal-800 hover:-translate-y-1 rounded-3xl cursor-pointer"
				>
